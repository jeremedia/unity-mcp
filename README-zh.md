> **Status audit (2026-06-20):** 通用 Unity MCP bridge 文档，已根据
> `MCPForUnity/package.json`、`Server/pyproject.toml`、Python 工具注册表、
> Unity 端工具处理器，以及当前客户端配置/服务器启动构建器重新核对。它不是
> CE 专用 Builder control-surface 权威；Python server tests 已通过
> `uv run --extra dev python -m pytest tests/ -q`，本轮未运行 Unity/client
> 运行时冒烟测试。
